    ...
    implements ThreadedPlugIn
{
  ...
  private final static String SRC_LAYER = "Source";
  private final static String MASK_LAYER = "Mask";
  ...
  private final static String UPDATE_SRC = "Update Source features with result";
  private final static String ADD_TO_SRC = "Add result to Source layer";
  private final static String CREATE_LYR = "Create new layer for result";
  ...
  public void run(TaskMonitor monitor, PlugInContext context)
    ...
    if ( (updateSource || addToSource)
          && ! srcLayer.isEditable()) {
      context.getWorkbenchFrame().warnUser(srcLayer.getName() + " is not editable");
      return;
    }

    monitor.report("Executing function " + functionToRun.getName() + "...");
    ...
    if (nArgs == 2) {
      ...
      if (fc2.size() != 1) {
        context.getWorkbenchFrame().warnUser("Mask must contain exactly one geometry");
        return;
      }
    ...
    if (modifiedFeatures.size() == 0) {
      context.getWorkbenchFrame().warnUser("No geometries were processed");
      return;
    }
...
//  private JCheckBox replaceSrcChkBox;
  ...
  private void setDialogValues(MultiInputDialog dialog, PlugInContext context)
  {
    ...
    dialog.setSideBarDescription(
        "Computes a geometric function on features in the Source layer.  "
        + "Geometry can be saved to a new layer, updated in place, or added to the Source layer.  "
    ...
    "The Source layer features provide the first operand for the chosen function",
    ...
    layer2ComboBox = dialog.addLayerComboBox(MASK_LAYER, maskLayer,
        "The Mask layer must contain a single feature, which is used as the second operand for binary functions",
        context.getLayerManager()  );
    ...
    updateSourceRB = dialog.addRadioButton(UPDATE_SRC, OUTPUT_GROUP, false,
        "Replace the geometry of Source features with the result geometry.  ");
