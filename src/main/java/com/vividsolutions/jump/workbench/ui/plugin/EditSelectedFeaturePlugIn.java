...
public class EditSelectedFeaturePlugIn extends WKTPlugIn {
	...
	public static MultiEnableCheck createEnableCheck(
		...
		return new MultiEnableCheck()
				...
				.add(
						checkFactory
								.createExactlyNFeaturesMustHaveSelectedItemsCheck(1));
	}

	public boolean execute(PlugInContext context) throws Exception {
		return execute(context, (Feature) context.getLayerViewPanel()
				.getSelectionManager().getFeaturesWithSelectedItems()
				.iterator().next(), true);
	}

	public boolean execute(PlugInContext context, Feature feature,
	...
	protected void apply(FeatureCollection c, PlugInContext context)
		...
		if (c.size() != 1) {
			throw new WorkbenchException("Expected 1 feature but found "
					+ c.size());
		}
		EditTransaction transaction = new EditTransaction(Arrays
				.asList(new Feature[] { feature }), getName(), layer,
				isRollingBackInvalidEdits(context), false, context
						.getWorkbenchFrame());
		//Can't simply pass the LayerViewPanel to the transaction because if
		// there is
		//an attribute viewer up and its TaskFrame has been closed, the
		// LayerViewPanel's
		//LayerManager will be null. [Jon Aquino]
		transaction.setGeometry(0, ((Feature) c.iterator().next())
				.getGeometry());
		transaction.commit();
	}
	...
	protected EnterWKTDialog createDialog(PlugInContext context) {
		...
		d
				.setTitle((layer(context).isEditable() ? "Edit " : "")
						+ "Feature "
						+ feature.getID()
						+ " In "
						+ layer
						+ (layer(context).isEditable() ? ""
								: " (layer is uneditable)"));
