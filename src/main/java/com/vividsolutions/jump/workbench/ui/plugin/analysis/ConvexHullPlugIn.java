...
public class ConvexHullPlugIn extends AbstractPlugIn implements ThreadedPlugIn {
    ...
    private void initDialog(PlugInContext context) {
        dialog = new MultiInputDialog(context.getWorkbenchFrame(), "Convex Hull", true);

        //dialog.setSideBarImage(IconLoader.icon("Overlay.gif"));
        dialog.setSideBarDescription(
            "Creates a new layer containing the convex hull of all the features in the source layer");
        ...
        throws Exception
    {
        ...
        context.addLayer(StandardCategoryNames.RESULT, "Convex Hull", hullFC);
    }

    private FeatureCollection convexHhull(TaskMonitor monitor, FeatureCollection fc) {
        monitor.allowCancellationRequests();
        monitor.report("Computing Convex Hull...");
