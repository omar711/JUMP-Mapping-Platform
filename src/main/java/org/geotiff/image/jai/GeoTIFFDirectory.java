...
public class GeoTIFFDirectory extends XTIFFDirectory
  ...
  /**
   * Constructs a GeoTIFFDirectory by reading a SeekableStream. The ifd_offset parameter specifies the stream offset
   * from which to begin reading; this mechanism is sometimes used to store private IFDs within a TIFF file that are not
  ...
  /**
   * Constructs a GeoTIFFDirectory from a SeekableStream. The directory parameter specifies which directory to read from
   * the linked list present in the stream; directory 0 is normally read but it is possible to store multiple images in
   * a single TIFF file by maintaing multiple directories.
   */
  public GeoTIFFDirectory(SeekableStream stream,
    int directory)
    throws java.io.IOException {
  ...
  private void createGeoTags() {
    ...
    Iterator fields = geoKeyIndex.values().iterator();
    ...
    while (fields.hasNext()) {
      XTIFFField geoKey = (XTIFFField) fields.next();
    ...
    if (tiffTag > 0) {
      ...
      if (values != null) {
        ...
        if (type == XTIFFField.TIFF_ASCII) {
          String svalue = values.getAsString(0).substring(valueOrOffset,
  ...
  /**
   * Returns an array of XTIFFFields containing all the fields in this directory. Prior to returning array, determine if
   * there are any GeoKeys, and if so, set up the corresponding GeoTIFF fields.
   */
  ...
  /**
   * populates the geoKeyIndex table from the values stored in the current TIFF fields.
   */
